...
namespace Qurre
{
	public static class Log
	{
		...
		public static void Info(object message)
		{
			...
			ServerConsole.AddLog($"[INFO] [{assembly.GetName().Name}] {message}", ConsoleColor.Green);
		}
		public static void Debug(object message)
		{
			if (!Debugging) return;
			Assembly assembly = Assembly.GetCallingAssembly();
			ServerConsole.AddLog($"[DEBUG] [{assembly.GetName().Name}] {message}", ConsoleColor.DarkYellow);
		}
		public static void Warn(object message)
		{
			Assembly assembly = Assembly.GetCallingAssembly();
			ServerConsole.AddLog($"[WARN] [{assembly.GetName().Name}] {message}", ConsoleColor.Yellow);
