...
namespace Qurre.API
{
	public static class Player
	{
		...
		public static MethodInfo SendSpawnMessage
		{
			get
			{
				if (sendSpawnMessage == null)
					sendSpawnMessage = typeof(NetworkServer).GetMethod("SendSpawnMessage", BindingFlags.Instance |
		...
		public static void SetUserId(this ReferenceHub player, string newId) =>
			player.characterClassManager.NetworkSyncedUserId = newId;
		...
		public static void SetPlayerId(this ReferenceHub player, int newId) =>
			player.queryProcessor.NetworkPlayerId = newId;
		...
		public static void Overwatch(this ReferenceHub player, bool over) =>
			player.serverRoles.CallTargetSetOverwatch(player.networkIdentity.connectionToClient, over);
		...
		public static void SetRole(this ReferenceHub player, RoleType newRole) =>
			player.characterClassManager.SetPlayersClass(newRole, player.gameObject);
		...
		public static Vector3 GetRotationVector(this ReferenceHub player) =>
			player.characterClassManager.transform.forward;

		public static void SetPosition(this ReferenceHub player, float x, float y, float z) =>
			player.playerMovementSync.OverridePosition(new Vector3(x, y, z), player.transform.rotation.eulerAngles.y);

		public static void SetPosition(this ReferenceHub player, Vector3 position) =>
			player.SetPosition(position.x, position.y, position.z);

		public static void SetRotation(this ReferenceHub player, Vector2 rotations) =>
			player.SetRotation(rotations.x, rotations.y);

		public static void SetRotation(this ReferenceHub player, float x, float y) =>
			player.playerMovementSync.NetworkRotationSync = new Vector2(x, y);
		...
		public static void SetGroup(this ReferenceHub player, string name, string color, bool show, string rankName)
		{
			if (ServerStatic.GetPermissionsHandler().GetAllGroups().ContainsKey(rankName))
			{
				...
				player.serverRoles.SetGroup(ServerStatic.GetPermissionsHandler().GetGroup(rankName), false, false,
					show);
			}
			else
			{
		...
		public static void SetGroup(this ReferenceHub player, UserGroup userGroup) =>
			player.serverRoles.SetGroup(userGroup, false, false, false);
		...
		public static GlobalBadge GetGlobalBadge(this ReferenceHub player)
		{
			...
			int badgeType = 0;
			if (int.TryParse(dictionary["Badge type"], out int type))
			{
				badgeType = type;
			}
		...
		private static void ShowTag(this ReferenceHub player, bool isGlobal = false) =>
			player.characterClassManager.CallCmdRequestShowTag(isGlobal);

		public static void RaMessage(this CommandSender sender, string message, bool success = true,
			string pluginName = null)
		{
			sender.RaReply((pluginName ?? Assembly.GetCallingAssembly().GetName().Name) + "#" + message, success, true,
				string.Empty);
		}

		public static void Broadcast(this ReferenceHub player, ushort time, string message,
			Broadcast.BroadcastFlags flag = 0) =>
			Map.BroadcastComponent.TargetAddElement(player.scp079PlayerScript.connectionToClient, message, time, flag);

		public static void ClearBroadcasts(this ReferenceHub player) =>
			Map.BroadcastComponent.TargetClearElements(player.scp079PlayerScript.connectionToClient);
		...
		public static void IntercomMute(this ReferenceHub player) =>
			player.characterClassManager.NetworkIntercomMuted = true;

		public static void IntercomUnmute(this ReferenceHub player) =>
			player.characterClassManager.NetworkIntercomMuted = false;

		public static bool IsIntercomMuted(this ReferenceHub player) =>
			player.characterClassManager.NetworkIntercomMuted;
		...
		public static void SetGodMode(this ReferenceHub player, bool enable) =>
			player.characterClassManager.GodMode = enable;
		...
		public static void Heal(this ReferenceHub player, float amount) => player.playerStats.Health =
			Mathf.Clamp(player.playerStats.Health + amount, 1, player.playerStats.maxHP);
		...
		{
			player.playerStats.HurtPlayer(
				new PlayerStats.HitInfo(amount, "WORLD", damageType, player.queryProcessor.PlayerId),
				player.gameObject);
		}
		...
		public static void SetAHP(this ReferenceHub player, float amount) =>
			player.playerStats.unsyncedArtificialHealth = amount;
		...
		public static float SetMaxAHP(this ReferenceHub player, int amount) =>
			player.playerStats.maxArtificialHealth = amount;

		public static List<Inventory.SyncItemInfo> GetAllItems(this ReferenceHub player) =>
			player.inventory.items.ToList();

		public static Inventory.SyncItemInfo GetCurrentItem(this ReferenceHub player) =>
			player.inventory.GetItemInHand();

		public static void SetCurrentItem(this ReferenceHub player, ItemType itemType) =>
			player.inventory.SetCurItem(itemType);

		public static void AddItem(this ReferenceHub player, ItemType itemType, float duration = float.NegativeInfinity,
			int sight = 0, int barrel = 0, int other = 0) =>
		...
		public static void AddItem(this ReferenceHub player, ItemType itemType) =>
			player.inventory.AddNewItem(itemType);
		...
		{
			player.inventory.SetPickup(item.id, item.durability, player.GetPosition(),
				player.inventory.camera.transform.rotation, item.modSight, item.modBarrel, item.modOther);
		...
		public static void Ban(this ReferenceHub player, int duration, string reason, string issuer = "API") =>
			player.gameObject.Ban(duration, reason, issuer);

		public static void Ban(this GameObject player, int duration, string reason, string issuer = "API") =>
			PlayerManager.localPlayer.GetComponent<BanPlayer>().BanUser(player, duration, reason, issuer, false);

		public static void Kick(this ReferenceHub player, string reason, string issuer = "API") =>
			player.Ban(0, reason, issuer);

		public static void Kick(this GameObject player, string reason, string issuer = "API") =>
			player.Ban(0, reason, issuer);
		...
		public static void SetScale(this ReferenceHub player, Vector3 scale) =>
			player.SetScale(scale.x, scale.y, scale.z);
		...
		public static void SetScale(this ReferenceHub player, float x, float y, float z)
		{
			try
			{
				...
				foreach (ReferenceHub target in GetHubs())
					SendSpawnMessage?.Invoke(null,
						new object[] {player.GetComponent<NetworkIdentity>(), target.GetConnection()});
			}
			catch (Exception exception)
			{
		...
		public static NetworkConnection GetConnection(this ReferenceHub player) =>
			player.scp079PlayerScript.connectionToClient;

		public static void Disconnect(this ReferenceHub player, string reason = null) =>
			ServerConsole.Disconnect(player.gameObject, string.IsNullOrEmpty(reason) ? "" : reason);

		public static void Kill(this ReferenceHub player, DamageTypes.DamageType damageType = default) =>
			player.playerStats.HurtPlayer(new PlayerStats.HitInfo(-1f, "WORLD", damageType, 0), player.gameObject);

		public static IEnumerable<ReferenceHub> GetHubs(this Team team) =>
			GetHubs().Where(player => player.GetTeam() == team);

		public static IEnumerable<ReferenceHub> GetHubs(this RoleType role) =>
			GetHubs().Where(player => player.GetRole() == role);

		public static string GetGroupName(this ReferenceHub player) =>
			ServerStatic.GetPermissionsHandler()._members()[player.GetUserId()];
		...
		public static void SetBypassMode(this ReferenceHub player, bool isEnabled) =>
			player.serverRoles.BypassMode = isEnabled;
		...
		public static void SetFriendlyFire(this ReferenceHub player, bool value) =>
			player.weaponManager.GetShootPermission(player.characterClassManager, value);
		...
		public static void PlayReloadAnimation(List<ReferenceHub> players)
		{
			foreach (ReferenceHub refHub in ReferenceHub.GetAllHubs().Values)
			{
				foreach (ReferenceHub ply in players)
				{
					refHub.weaponManager.RpcReload(0);
				}
			}
		}
