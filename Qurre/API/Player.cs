...
namespace Qurre.API
{
    public class Player
    {
        ...
        public Radio Radio
        { get { if (radio == null) { radio = GameObject.GetComponent<Radio>(); } return radio; } }

        public Escape Escape
        { get { if (escape == null) { escape = ClassManager.GetComponent<Escape>(); } return escape; } }
        ...
        public static Player Get(string args)
        {
            try
            {
                ...
                if (ArgsPlayers.TryGetValue(args, out Player playerFound) && playerFound?.ReferenceHub != null)
                    return playerFound;
                if (short.TryParse(args, out short id))
                    return Get(id);
        ...
        {
            Controllers.Broadcast bc = new Controllers.Broadcast(this, message, time);
        ...
        public bool Damage(float damage, string deathReason) => PlayerStats.DealDamage(new CustomReasonDamageHandler(deathReason, damage));

        public bool Damage(float damage, DeathTranslation deathReason) => PlayerStats.DealDamage(new UniversalDamageHandler(damage, deathReason));
        ...
        public Item AddItem(ItemType itemType)
        {
            ...
            {
                if (AttachmentsServerHandler.PlayerPreferences.TryGetValue(ReferenceHub, out Dictionary<ItemType, uint> _d) && _d.TryGetValue(itemType, out uint _y))
        ...
        public void ClearInventory()
        {
            for (int i = 0; i <= AllItems.Count(); i++)
                RemoveItem(AllItems.ElementAt(i), true);
        }
        ...
        public void EnableEffect(PlayerEffect effect, float duration = 0, bool addDurationIfActive = false) => PlayerEffectsController.EnableEffect(effect, duration, addDurationIfActive);

        public T GetEffect<T>() where T : PlayerEffect => PlayerEffectsController.GetEffect<T>();
        ...
        private Team GetTeam(RoleType rt) => rt switch
        {
        ...
        private Side GetSide(Team team) => team switch
        {
            Team.SCP => Side.SCP,
        ...
        internal void CheckEscape()
        {
            ...
            Events.EscapeEvent ev = new Events.EscapeEvent(this, newRole);
