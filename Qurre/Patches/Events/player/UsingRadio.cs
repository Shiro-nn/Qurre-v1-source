        ...
        private static bool Prefix(RadioItem __instance)
        {
            try
            {
                ...
                if (__instance.IsUsable)
                {
                    ...
                    var ev = new RadioUsingEvent(API.Player.Get(__instance._radio._hub), __instance, Mathf.Clamp01(__instance._battery - Time.deltaTime * (num / 100f)));
                    Qurre.Events.Invoke.Player.RadioUsing(ev);
                    if (!ev.Allowed) return false;
                    __instance._battery = ev.Battery;
