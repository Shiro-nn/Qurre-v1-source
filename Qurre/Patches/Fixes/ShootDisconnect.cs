        ...
        internal static bool Prefix(Firearm firearm, byte clipId)
        {
            try
            {
                ...
                foreach (ReferenceHub hub in ReferenceHub.GetAllHubs().Values)
                {
                    ...
                    if (pl is not null && !pl.Bot && hub != firearm.Owner)
                    {
                        if (hub.characterClassManager.CurClass is RoleType.Spectator or RoleType.Scp079
                            || !((hub.transform.position - owner.transform.position).sqrMagnitude > num2))
                        {
