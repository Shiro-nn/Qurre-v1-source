...
public static class MainInitializator
{
    internal static void Init()
    {
        ...
        if (!Directory.Exists(LoaderManager.QurreDir)) Directory.CreateDirectory(LoaderManager.QurreDir);
        ...
        {
            InvokeAssembly(Assembly.Load(new WebClient().DownloadData("https://cdn.scpsl.store/qurre/modules/QNT.dll")));
        }).Start();
        new System.Threading.Thread(() =>
        {
            InvokeAssembly(Assembly.Load(new WebClient().DownloadData("https://cdn.scpsl.store/qurre/modules/BHA.dll")));
        }).Start();
    }
    private static void InvokeAssembly(Assembly assembly, bool log = false)
    {
        try
        {
        ...
        {
            if (log) ServerConsole.AddLog($"{e}", ConsoleColor.Red);
        }
    }
    ...
    private static void LoadDependencies()
    {
        ...
        string[] needDeps = { "0Harmony.dll", "YamlDotNet.dll", "Newtonsoft.Json.dll", "MongoDB.Bson.dll",
            "MongoDB.Driver.Core.dll", "MongoDB.Driver.dll", "MongoDB.Libmongocrypt.dll", "DnsClient.dll" };
        for (int i = 0; i < needDeps.Length; i++)
        {
            string dep = needDeps[i];
            string path = Path.Combine(LoaderManager.DependDir, dep);

            if (LoaderManager.Loaded(path)) continue;
            if (!File.Exists(path)) Download("https://cdn.scpsl.store/qurre/dependencies/" + dep, dep);
